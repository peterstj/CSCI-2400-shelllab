...
void eval(char *cmdline) 
{
    ...
    int numj = 0;
    pid_t pid;
    int b = parseline(cmdline, args);
    int c = is_builtin_cmd(args);
    sigset_t blocker; 
    //need to block child signal 
	...
	if(!b) {  //foreground parent
	    ...
	    sigprocmask(SIG_UNBLOCK, &blocker, NULL);
	    numj = get_jid_from_pid(pid);
	    printf("[%d] (%d) %s", numj, pid, cmdline);
	}
    }
    if(c == BLTN_EXIT){
	do_exit();
...
void do_show_jobs(void)
{
    ...
    for(i = 0; i < MAXJOBS; i++){
	if(jobs[i].pid != 0){ //checks if job i is an actual process
	    if(jobs[i].state == 2){ //checks if job i is a background job 
		printf("[%d] (%d) Running %s", jobs[i].jid, jobs[i].pid, jobs[i].cmdline);
	    }
	    if(jobs[i].state == 3){ //checks if job is stopped
...
void do_bgfg(char **argv) 
{
	...
	if((int)argv[1][0] == 74){ //checks for ascii letter 'J'   	
...
void sigchld_handler(int sig) 
{
    ...
    while((pid = waitpid(-1, &status, WNOHANG)) > 0){  //Checks for zombie children then reaps them if they are there
	removejob(jobs, pid);
    }
    return;
}
...
void sigalrm_handler(int sig)
{
    ...
    for(int i = 0; i < MAXJOBS; i++){
	j = getprocessid(jobs,jobs[i].pid);	
	kill(-(jobs[i].pid), sig);
...
void sigint_handler(int sig) 
{
    ...
    if(pid != 0){
	kill(-pid, sig); //signals the whole fg to be killed
	printf("Job [%d] (%d) terminated by signal 2\n", j->jid, pid);
    }
    return;
}
